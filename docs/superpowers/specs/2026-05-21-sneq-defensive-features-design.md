...
L'issue #1 du repo `sneq-narrative-system` rapporte quatre bugs de continuité observés lors d'une session RP du 21/05/2026 ("La Tour de Jade", campagne Skyrim). Diagnostic après échange avec Hermes-Agent (l'agent MJ tournant sur Discord) :
...
| Mira témoigne d'un événement jamais joué | Pas d'appel à `get-relevant-facts(mira)`. Mira n'est pas non plus enregistrée dans SNEQ — `get-relevant-facts` aurait retourné vide. |
...
Réutilise `test/fixtures/`. Si besoin d'un fixture campaign avec entités prédéfinies (Alduin, Mira, Dragonsreach), l'ajouter dans `test/fixtures/tour-de-jade.fixture.ts` — mais probablement réutilisable depuis les fixtures CLI existants.
