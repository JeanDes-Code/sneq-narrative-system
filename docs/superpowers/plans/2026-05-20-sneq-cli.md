...
**Goal:** Add a `sneq-engine` CLI binary that wraps the existing `@sneq/engine` library, exposing 12 commands (10 tools + `init-campaign` + `get-scene`) with JSON-only I/O for out-of-process agent consumption (Hermes-Agent).
...
- **Hermes integration**: not in this plan. Once the CLI lands, the Hermes-Agent runtime can shell out via `child_process.execFile`. That work belongs to its own repo, not this one.
