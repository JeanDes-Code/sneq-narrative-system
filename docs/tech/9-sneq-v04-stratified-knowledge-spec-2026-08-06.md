...
`get-relevant-facts` → `get-holder-context`; `register-fact` → `commit-narrative`; `advance-turn --days N`; `prepare-turn --holder <id>`; add `upsert-holder`, `show-dispatch-policy` and `set-dispatch-policy` (#15 — same special-case routing as `upsert-holder` below; the tool surface stays at ten). Everything else unchanged.

**The count:** 14 today (`src/cli/types.ts:5-20`), two renames (count-neutral) plus three additions = **17**. README:210 and UPGRADING:175 both encode the old 14 — UPGRADING:175 as an *executable* verification step, so it fails on release day if not updated.
