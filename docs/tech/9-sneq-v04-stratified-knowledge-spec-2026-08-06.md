...
Phase A also closed §2.6's ingress hole in the shape §11.2 described: `commit_narrative` gains `playerUtterance`, the engine runs `detectUptake` inside the commit, and caller-supplied `PLAYER_UPTAKE` evidence cannot outrank it. Before this, `promotionEvidence[]` was entirely caller-supplied and the model decided its own promotions — precisely what §2.6 forbids. *Resolved 2026-09-03 ([#52](https://github.com/JeanDes-Code/sneq-narrative-system/issues/52)): the guard did not exist as written. The merge deduped by `inventionId` and never read the kind, so detection outranked a caller-supplied `PLAYER_UPTAKE` only when the engine had detected the same uptake; for an invention it had not detected, the entry passed through and promoted. #52 makes the engine refuse a caller-supplied `PLAYER_UPTAKE` outright, whole bundle included.*
